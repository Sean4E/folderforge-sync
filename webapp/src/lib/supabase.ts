...
if (!supabaseUrl || !supabaseAnonKey) {
  console.warn('Supabase credentials not found. Set VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY in your .env file');
}
...
export interface TemplateChange {
  ...
  change_type: 'folder_add' | 'folder_rename' | 'folder_delete' | 'folder_move' | 'folder_type_change' | 'template_update';
...
// Sample Templates Types
export interface SampleTemplate {
  id: string;
  name: string;
  description: string | null;
  category: 'web' | 'mobile' | 'backend' | 'devops' | 'design' | 'general';
  icon_type: string;
  color: string;
  folder_structure: FolderSnapshot[];
  tags: string[];
  popularity: number;
  is_featured: boolean;
  created_at: string;
