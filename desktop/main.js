...
function updateTrayMenu() {
  ...
  const contextMenu = Menu.buildFromTemplate([
    ...
    { type: 'separator' },
    {
      ...
      click: () => {
        shell.openExternal('https://folderforge.app');
      },
    },
    {
      label: 'Settings',
 ...
 */
function detectNamingPatterns(folderNames) {
  const patterns = {
  ...
  // Regex patterns
  const numericPrefixRegex = /^(\d{1,3})[_\-\s]/;
  const datePrefixRegex = /^(\d{4}[-_]\d{2}(?:[-_]\d{2})?)[_\-\s]/;
  const categoryPrefixes = ['WIP', 'DRAFT', 'FINAL', 'ARCHIVE', 'OLD', 'NEW', 'TEMP', 'BACKUP'];
  const versionSuffixRegex = /[_\-\s]v(\d+(?:\.\d+)?)$/i;

  folderNames.forEach(name => {
    // Check numeric prefix (01_, 02_, etc.)
    if (numericPrefixRegex.test(name)) {
      patterns.numericPrefix.count++;
      if (patterns.numericPrefix.examples.length < 3) {
        patterns.numericPrefix.examples.push(name);
      }
    }

    // Check date prefix (2024-01_, 2024-01-15_, etc.)
    if (datePrefixRegex.test(name)) {
      patterns.datePrefix.count++;
      if (patterns.datePrefix.examples.length < 3) {
        patterns.datePrefix.examples.push(name);
      }
    }

    // Check category prefix (WIP_, FINAL_, etc.)
    const upperName = name.toUpperCase();
    if (categoryPrefixes.some(cat => upperName.startsWith(cat + '_') || upperName.startsWith(cat + '-'))) {
      patterns.categoryPrefix.count++;
      if (patterns.categoryPrefix.examples.length < 3) {
        patterns.categoryPrefix.examples.push(name);
      }
    }

    // Check version suffix (_v1, _v2, etc.)
    if (versionSuffixRegex.test(name)) {
      patterns.versionSuffix.count++;
      if (patterns.versionSuffix.examples.length < 3) {
        patterns.versionSuffix.examples.push(name);
      }
    }
  });

  // Calculate confidence scores (percentage of folders matching each pattern)
  Object.keys(patterns).forEach(key => {
    patterns[key].confidence = Math.round((patterns[key].count / total) * 100);
  });
...
/**
 * Recursively scan a directory and build folder structure
 */
async function scanDirectory(dirPath, depth = 0, maxDepth = 10) {
  ...
  try {
    ...
    // Filter to only directories, ignoring hidden folders and node_modules
    ...
    for (const dir of directories) {
      ...
      const child = await scanDirectory(childPath, depth + 1, maxDepth);
...
/**
 * Count total folders and max depth in a tree
 */
function countFoldersAndDepth(node, currentDepth = 0) {
  let count = 1;
  let maxDepth = currentDepth;

  if (node.children && node.children.length > 0) {
    for (const child of node.children) {
      const result = countFoldersAndDepth(child, currentDepth + 1);
      count += result.count;
...
/**
 * Collect all folder names at a given depth
 */
function collectFolderNamesAtDepth(node, targetDepth, currentDepth = 0) {
  const names = [];
  ...
  if (node.children && currentDepth < targetDepth) {
    for (const child of node.children) {
      names.push(...collectFolderNamesAtDepth(child, targetDepth, currentDepth + 1));
    }
  }
...
// IPC handler for scanning directories
ipcMain.handle('scan-directory', async (event, directoryPath) => {
  try {
    ...
    // Scan the directory
    const structure = await scanDirectory(directoryPath);

    // Count folders and depth
    const { count: totalFolders, maxDepth } = countFoldersAndDepth(structure);

    // Detect patterns at each level
    const patternsByLevel = {};
    for (let depth = 0; depth <= Math.min(maxDepth, 3); depth++) {
      const namesAtDepth = collectFolderNamesAtDepth(structure, depth);
      if (namesAtDepth.length > 1) {
        patternsByLevel[depth] = detectNamingPatterns(namesAtDepth);
      }
    }
...
// IPC handler for browsing/selecting directory for import
